...
export default function EditAdminDepartment({ params }) {
  const { id } = params;
  ...
  }, [user, authLoading, router, isAdmin, id]);
  ...
  const fetchDepartment = async () => {
    try {
      ...
      const adminDepsResponse = await fetch('/api/admin/departments');
      if (!adminDepsResponse.ok) {
        throw new Error('Failed to verify department access');
      }
      const adminDeps = await adminDepsResponse.json();
      const hasAccess = adminDeps.some(dep => dep._id === id);
      ...
      setFormData({
        ...
        headId: data.head ? data.head._id : '',
      });
      setLoading(false);
    } catch (err) {
